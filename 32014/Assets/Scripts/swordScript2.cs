...
public class swordMelee : MonoBehaviour
{
    ...
    void Update()
    {
        if (startTimer == true && pm.GameIsPaused == false)
        {
            ...
            if (timer > 0.5)
            {
                ...
                transform.position = new Vector3(transform.position.x, hdp.transform.position.y + 0.7f, transform.position.z);
    ...
    public void attack()
    {
        ...
        startTimer = true;
        duringattack = true;
        transform.position = new Vector3(transform.position.x, hdp.transform.position.y-1, transform.position.z);
        if (mad.playerDirection == true)
        {
